...
export class LoginController {
    ...
    loginAdmin = (req: express.Request, res: express.Response) => {
        ...
        UserModel.findOne({ 'username': username, 'password': password, "type": "admin" }).then((user) => {
            res.json(user);
        }).catch(err => {
            console.log(err)
        })
    ...
    changePassword = (req: express.Request, res: express.Response) => {
        ...
        UserModel.findOne({ 'username': username, 'password': oldPassword }).then((user) => {
            if (!user) {
                res.json("Greska");
            }
            else {
                UserModel.updateOne({ 'username': username }, { 'password': newPassword }).then((ok) => {
                    res.json("ok");
                })
            }
        })
    ...
    getOwnerCount = (req: express.Request, res: express.Response) => {
        UserModel.countDocuments({ type: 'owner', state: 'active' }).then((count) => {
            res.json(count);
        }).catch(err => {
            console.log(err);
        })
    ...
    getDecoratorCount = (req: express.Request, res: express.Response) => {
        UserModel.countDocuments({ type: 'decorator', state: 'active' }).then((count) => {
            res.json(count);
        }).catch(err => {
            console.log(err);
        })
    ...
    getGardenCount = (req: express.Request, res: express.Response) => {
        RenovationModel.countDocuments({ "finished": true }).then((count) => {
            res.json(count);
        }).catch(err => {
            console.log(err);
        })
    ...
    get24jobCount = async (req: express.Request, res: express.Response) => {
        const now = new Date();
        const last24h = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        try {
            const renovationCount = await RenovationModel.countDocuments({
                now: { $gte: last24h }
            });

            const odrzavanjeCount = await OdrzavanjeModel.countDocuments({
                now: { $gte: last24h }
            });

            const totalCount = renovationCount + odrzavanjeCount;
            res.json(totalCount);
        } catch (err) {
            console.log(err);
    ...
    get7jobCount = async (req: express.Request, res: express.Response) => {
        const now = new Date();
    ...
    get30jobCount = async (req: express.Request, res: express.Response) => {
        const now = new Date();
        const last7d = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
